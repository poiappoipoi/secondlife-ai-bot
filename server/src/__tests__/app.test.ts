...
describe('App', () => {
  ...
  it('should have routes registered', () => {
    ...
    const hasChatRoute = routes.some((r: any) => r.path === '/chat' || r.regexp?.test('/chat'));
    const hasSystemPromptRoute = routes.some(
      (r: any) => r.path === '/SetSystemPrompt' || r.regexp?.test('/SetSystemPrompt')
    );
